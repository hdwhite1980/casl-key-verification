...
export class CASLVerification extends HTMLElement {
  ...
  initializeState() {
    ...
    this.unsubscribeHandlers.push(
      stateManager.subscribe('formData', state => {
        ...
        if (fieldsChanged) {
          this.validateFormWithButtonUpdate();
    ...
    this.unsubscribeHandlers.push(
      stateManager.subscribe('ui', state => {
        ...
        if (this.isLoading !== state.loading || this.apiError !== state.alert) {
          ...
          this.updateButtonStates(); // Update buttons immediately when loading state changes
          this.render();
        }
      })
    );
    ...
    // Set up initial button states after state initialization
    setTimeout(() => {
      this.validateFormWithButtonUpdate();
    }, 100);
  }
   ...
   * This provides instant feedback for form validation
   */
  updateButtonStates() {
    const shadowRoot = this.shadowRoot;
    if (!shadowRoot) return;
    
    // Find buttons using multiple selectors for reliability
    const nextBtn = shadowRoot.querySelector('.btn-next') || 
                   shadowRoot.querySelector('button[onclick*="handleNextStep"]') ||
                   Array.from(shadowRoot.querySelectorAll('button'))
                     .find(btn => btn.textContent.includes('Next') || btn.textContent.includes('Submit'));
    
    const prevBtn = shadowRoot.querySelector('.btn-previous') ||
                   shadowRoot.querySelector('button[onclick*="handlePreviousStep"]') ||
                   Array.from(shadowRoot.querySelectorAll('button'))
                     .find(btn => btn.textContent.includes('Previous'));
    
    if (nextBtn) {
      const shouldEnable = this.isFormValid && !this.isLoading;
      nextBtn.disabled = !shouldEnable;
      nextBtn.setAttribute('aria-disabled', !shouldEnable);
      ...
      console.log(`Next button ${shouldEnable ? 'enabled' : 'disabled'} - Valid: ${this.isFormValid}, Loading: ${this.isLoading}`);
    }
    
    if (prevBtn) {
      const shouldEnable = this.currentStep > 0 && !this.isLoading;
      prevBtn.disabled = !shouldEnable;
      prevBtn.setAttribute('aria-disabled', !shouldEnable);
  ...
  /**
   * Enhanced validation that immediately updates button states
   */
  validateFormWithButtonUpdate() {
    // Run normal validation
    this.validateForm();
    
    // Immediately update button states for responsive UI
    this.updateButtonStates();
    
    // Log validation state for debugging
    console.log(`Step ${this.currentStep} validation:`, {
      errors: this.errors,
      isValid: this.isFormValid,
      errorCount: Object.keys(this.errors).filter(key => this.errors[key]).length
    });
  }
  ...
  /**
   * Enable debug mode for button states
   */
  enableDebugButtonStates() {
    // Debug helper to track button state changes
    if (window.location.search.includes('debug=true')) {
      // Monitor form data changes
      const originalUpdateFormData = stateManager.updateFormData;
      stateManager.updateFormData = (data) => {
        console.log('🔄 Form data updated:', data);
        const result = originalUpdateFormData.call(stateManager, data);
        
        // Trigger button state update after state change
        setTimeout(() => {
          this.validateFormWithButtonUpdate();
        }, 0);
  ...
  render() {
    ...
    // After rendering, always ensure button states are correct
    setTimeout(() => {
      this.updateButtonStates();
    }, 0);
  ...
  /**
   * Handle input changes with immediate button state updates
  ...
  handleInputChange(event) {
    ...
    // CRITICAL: Always validate and update buttons immediately
    this.validateFormWithButtonUpdate();
    
    // For text inputs, also debounce full render for performance
    if (type === 'text' || type === 'email' || type === 'tel' || type === 'url') {
      ...
      // For non-text inputs, render immediately since they're less frequent
      this.render();
    }
    
    // Special handling for background check consent
